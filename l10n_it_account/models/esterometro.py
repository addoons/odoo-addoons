...
class ComunicazioneDatiIva(models.Model):
    ...
    def _prepare_cedente_partner_id(self, partner, vals=None):
        ...
        vals['cedente_sede_Indirizzo'] = '{} {}'.format(
            ...
            partner.street2 and partner.street2 or '').strip()
    ...
    def _get_fatture_emesse_domain(self):
        ...
        posizioni_esterometro = self.env['account.fiscal.position'].sudo().search([('esterometro', '=', True)]).ids
        domain = [('type', 'in', ['out_invoice', 'out_refund']),
                  ('comunicazione_dati_iva_escludi', '=', False),
                  ('move_id', '!=', False),
                  ('fiscal_position_id', 'in', posizioni_esterometro),
    ...
    def _check_errors_dte(self):
        ...
        for partner in comunicazione.fatture_emesse_ids:
            ...
            if not check_normalized_string(
                partner.cessionario_sede_NumeroCivico
            ):
                errors.append(_(
